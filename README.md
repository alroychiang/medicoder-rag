...
**[→ Browse the evaluation results](https://alroychiang.github.io/medical-code-agent/demo/)**
— all 54 cases, each with its clinical note, the codes the pipeline predicted, and
which ground-truth codes it hit or missed.
...
├── fiftyfour_cases_retrieval_only.py  # Version A baseline (no selection LLM)
├── build_demo_data.py    # merge notes + results -> render the results viewer
├── requirements.txt
├── data/
│   ├── icd10_cm_cases.json   # the 54 evaluation cases
│   ├── sample_cases.json     # synthetic demo cases (committed)
│   └── results_full.json     # evaluation output (committed as evidence)
├── demo/                 # published at github.io/medical-code-agent/demo/
│   ├── template.html         # source — edit this
│   └── index.html            # generated by build_demo_data.py — do not edit
└── docs/
    └── section2_design.pdf   # full design write-up
```

### Rebuilding the results viewer

`demo/index.html` is generated: it inlines the merged case data, so the page is a
single self-contained file with no server or fetch. After re-running the pipeline
(or editing `demo/template.html`):

```bash
python3 build_demo_data.py
```

This merges `data/icd10_cm_cases.json` with `data/results_full.json`, verifies the
two line up case-for-case, and re-renders `demo/index.html`.
